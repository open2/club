...
// 클럽2 - 기본설정 (클럽 게시글만 가능하게)
$gr_id = $nc['gr_id'];

// ----------------------------------------
// 아래의 코드는 bbs/search.php를 수정한 것 입니다.
// bbs/search.php가 업데이트 되면 반드시 같이 수정되어야 합니다.
// ----------------------------------------

//if (!$stx) alert("검색어가 없습니다."); 

$g4[title] = "검색 : " . $stx;
include_once("$nc[cb_path]/bbs/_head.php");
...
if ($stx)
{
    ...
    // 클럽2 - 모든 클럽 게시판이 검색 가능하게 수정
    // $sql = " select gr_id, bo_table, bo_read_level from $g4[board_table] where bo_use_search = '1' and bo_list_level <= '$member[mb_level]' ";
    $sql = " select gr_id, bo_table, bo_read_level from $g4[board_table] where bo_list_level <= '$member[mb_level]' ";
    ...
    for ($idx=$table_index; $idx<count($search_table); $idx++) 
    {
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) 
        {
            ...
            // 클럽2 - 게시글 index를 변경
            $list[$idx][$i][href] = "$g4[bbs_path]/board.php?bo_table=$search_table[$idx]&wr_id=$row[wr_parent]";
            ...
            $subject = get_text(strip_tags($row[wr_subject]));
...
/* 클럽2- 그룹선택을 할 수 없게 (select box는 없애고, hidden type으로
...
$group_select .= "</select>";
*/
$group_select .= "<input type='hidden' name='gr_id' id='gr_id' value=''>";
...
include_once("$nc[cb_path]/bbs/_tail.php");
?>
