...
if (!$board[bo_1]) alert("게시판 설정 : 여분 필드 1 에 목록에서 보여질 이미지의 폭(높이)을 설정하십시오. (픽셀 단위)");
...
if ($mod) $mod = 3; // 아무런 설정이 없으면 기본으로 3을 지정
...
<tr height="25">
    ...
    <? if ($is_category) { ?>
    <select name=sca onchange="location='<?=$category_location?>'+this.value;"><option value=''>전체</option><?=$category_option?>
    </select> 
    <? } ?>
